...
public class GoChatMethodHintProvider extends BaseChatMethodHintProvider {
        ...
        return new FactoryInlayHintsCollector(editor) {
            ...
            public boolean collect(@NotNull PsiElement element, @NotNull Editor editor, @NotNull InlayHintsSink sink) {
                ...
                if (!PsiUtils.instanceOf(element,
                        "com.intellij.psi.PsiMethod", "com.goide.psi.GoMethodDeclaration", "com.goide.psi.GoFunctionDeclaration")
                        || PsiUtils.instanceOf(element, "com.intellij.psi.PsiTypeParameter")) {
                    return true;
                }
                if (PsiUtils.instanceOf(element, "com.intellij.psi.PsiMethod")
                        && PsiUtils.isInvalidJavaMethod(element)) {
                    return true;
                }
                PresentationFactory factory = getFactory();
                Document document = editor.getDocument();
                int offset = GoChatMethodHintProvider.getAnchorOffset(element);
                ...
                smartList.add(factory.textSpacePlaceholder(5, true));
                SequencePresentation shiftedPresentation = new SequencePresentation(smartList);
                InlayPresentation finalPresentation = factory.referenceOnHover(shiftedPresentation, (event, translated) -> GoChatMethodHintProvider.this.onClick(editor, element, event));
                sink.addInlineElement(startOffset, true, finalPresentation, true);
                return true;
            }
        };
    }
