...
public class JavaChatMethodHintProvider extends BaseChatMethodHintProvider {
    ...
    public InlayHintsCollector getCollectorFor(@NotNull PsiFile psiFile, @NotNull Editor editor, @NotNull ChatMethodHintSettings chatHintSettings, @NotNull InlayHintsSink inlayHintsSink) {
        return new FactoryInlayHintsCollector(editor) {
            ...
            public boolean collect(@NotNull PsiElement element, @NotNull Editor editor, @NotNull InlayHintsSink sink) {
                ...
                int offset = JavaChatMethodHintProvider.getAnchorOffset(element);
                int line = document.getLineNumber(offset);
                int startOffset = document.getLineStartOffset(line);
                String linePrefix = editor.getDocument().getText(new TextRange(startOffset, offset));
                offset += JavaChatMethodHintProvider.this.findRealOffsetBySpace(editor, linePrefix);
                ...
                InlayPresentation finalPresentation = factory.referenceOnHover(shiftedPresentation, (event, translated) -> JavaChatMethodHintProvider.this.onClick(editor, element, event));
