package com.ppdai.bicoder.chat.diff.handler;
...
import com.ppdai.bicoder.chat.diff.MyBaseDiffConfig;
...
public class BaseDiffHandler {
    ...
    public BaseDiffHandler(Project project, String messageId) {
    ...
    private void handleCode(String code) {
        var currentDocument = baseDiffEditor.getDocument();
        Application application = ApplicationManager.getApplication();
        String finalCode = handleCodeIndent(code);
        application.invokeLater(
                () -> application.runWriteAction(() ->
                        WriteCommandAction.runWriteCommandAction(project, TriggerCompleteCommand.HANDLE_CHAT_BASE_DIFF_CODE_COMMAND, null, () -> {
                            currentDocument.replaceString(startOffset, startOffset + lastCodeLength, finalCode);
                            lastCodeLength = finalCode.length();
                            baseDiffEditor.getCaretModel().moveToOffset(startOffset + finalCode.length());
                            baseDiffEditor.getComponent().revalidate();
                            baseDiffEditor.getComponent().repaint();
                        })));
    }
    ...
    public void init() {
        ...
        baseDiffEditor = EditorUtils.copyEditorAndDeleteSelectedCode(project, selectedTextEditor, startOffset, textRange.getEndOffset());
        hasInit = true;
        openDiffWindow();
    }
    ...
    private void openDiffWindow() {
        ...
        DiffContent suggestDiffContent = diffContentFactory.create(project, FileUtils.getEditorFile(baseDiffEditor));
        ...
        request.putUserData(MyBaseDiffConfig.NEED_ACCEPT_AND_REJECT, true);
        request.putUserData(MyBaseDiffConfig.SET_RENDER_COMPONENT, this::setRenderComponent);
        application.invokeLater(
                () -> application.runWriteAction(() ->
    ...
    private void acceptEdit() {
        ...
        application.invokeLater(
                () -> application.runWriteAction(() ->
                        WriteCommandAction.runWriteCommandAction(project, TriggerCompleteCommand.ACCEPT_CHAT_BASE_DIFF_CODE_COMMAND, null, () -> {
                            //替换文本
                            Document selectedTextDocument = selectedTextEditor.getDocument();
                            Document editCodeDocument = baseDiffEditor.getDocument();
