...
public class MyTestsSchemaExtension extends DiffExtension {
    @Override
    public void onViewerCreated(FrameDiffTool.@NotNull DiffViewer viewer, @NotNull DiffContext context, @NotNull DiffRequest request) {
        Boolean isNeed = request.getUserData(MyTestsSchemaConfig.NEED_ACCEPT_AND_REJECT);
        if (isNeed != null && isNeed) {
            if (viewer instanceof TwosideTextDiffViewer) {
                ...
                Consumer<JComponent> setRenderComponent = request.getUserData(MyTestsSchemaConfig.SET_RENDER_COMPONENT);
